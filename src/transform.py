...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    data = pd.merge(pd.merge(orders, items, on='order_id', how='inner'),
    ...
    aggregations = delivered.groupby('order_id').agg({'freight_value':'sum',
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    'order_purchase_timestamp').agg({'order_id':'count'})

    order_purchase_ammount_per_date = order_purchase_ammount_per_date.groupby(
        order_purchase_ammount_per_date.index.date).sum()
    ...
    result_df = result_df = DataFrame(order_purchase_ammount_per_date)
